...
- You can optionally enable `Setup controllers` option during animation import to automatically create basic controllers and IK setup (WIP feature, only works for 1P as of now). By default, every controller bone will have no parent, meaning that some weapon parts will be "detached" from each other, you can use `Pose -> BF2 -> Change Parent` to fix that without breaking the keyframes.
