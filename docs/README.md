...
- You can optionally enable `Setup controllers` option during animation import to automatically create basic controllers and IK setup (WIP feature, only works for 1P as of now)
...
# Mesh import/export
There are two ways of importing an BF2 mesh into Blender. One is to use `Import -> BF2` menu to directly import a specific mesh file type e.g. `StaticMesh`. This however will only import the "raw" mesh data according to its internal file structure lacking information about objectTemplate's metadata such as geometry part names, their position/rotation or hierarchy. This is fine for simple meshes in most cases but a preferable way of importing a mesh is to use `Import -> ObjecTemplate (.con)` which will parse the objectTemplate definition and import the visible geometry of the object (optionally also its collision mesh), split all mesh parts into sub-meshes, reposition them as well as recreate their hierarchy. For re-exporting, always use the corresponding option from the `Export -> BF2` menu.
