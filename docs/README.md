...
- You can automatically create basic controllers and IK setup either when making an animation from scratch (use `Pose -> BF2 -> Setup Controlers`) or during animation import (use `Setup controllers` option) (WIP feature, only works for 1P as of now). By default, every controller bone will have no parent, meaning that some weapon parts will be "detached" from each other, you can use `Pose -> BF2 -> Change Parent` to fix that without messing up the keyframed positions/roations.
